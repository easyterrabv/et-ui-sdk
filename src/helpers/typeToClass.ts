...
const buttonTypeHoverMap: Map<UI_TYPES, string> = new Map([
    [
        UI_TYPES.DEFAULT,
        `${commonButtonHoverClasses} button-type__default__hover`
    ],
    [
        UI_TYPES.PRIMARY,
        `${commonButtonHoverClasses} button-type__primary__hover`
    ],
    [
        UI_TYPES.SUCCESS,
        `${commonButtonHoverClasses} button-type__success__hover`
    ],
    [UI_TYPES.DANGER, `${commonButtonHoverClasses} button-type__danger__hover`],
    [
        UI_TYPES.WARNING,
        `${commonButtonHoverClasses} button-type__warning__hover`
    ],
    [
        UI_TYPES.PREMIUM,
        `${commonButtonHoverClasses} button-type__premium__hover`
    ]
]);
...
const buttonTypeDisabledMap: Map<UI_TYPES, string> = new Map([
    [
        UI_TYPES.DEFAULT,
        `${commonButtonDisabledClasses} button-type__default__disabled`
    ],
    [
        UI_TYPES.PRIMARY,
        `${commonButtonDisabledClasses} button-type__primary__disabled`
    ],
    [
        UI_TYPES.SUCCESS,
        `${commonButtonDisabledClasses} button-type__success__disabled`
    ],
    [
        UI_TYPES.DANGER,
        `${commonButtonDisabledClasses} button-type__danger__disabled`
    ],
    [
        UI_TYPES.WARNING,
        `${commonButtonDisabledClasses} button-type__warning__disabled`
    ],
    [
        UI_TYPES.PREMIUM,
        `${commonButtonDisabledClasses} button-type__premium__disabled`
    ]
]);
...
export const typeToButtonClass = (type: UI_TYPES): string => {
    return (
        (buttonTypeMap.has(type)
            ? buttonTypeMap.get(type)
            : buttonTypeMap.get(UI_TYPES.DEFAULT)) || ""
    );
};
...
export const typeToButtonDisabledClass = (type: UI_TYPES): string => {
    return (
        (buttonTypeDisabledMap.has(type)
            ? buttonTypeDisabledMap.get(type)
            : buttonTypeDisabledMap.get(UI_TYPES.DEFAULT)) || ""
    );
};

export const sizeToClass = (size: UI_SIZING): string => {
    return (
        (sizeMap.has(size) ? sizeMap.get(size) : sizeMap.get(UI_SIZING.M)) || ""
    );
};
